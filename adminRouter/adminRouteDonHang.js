        ...
        const topVariants = await DonHang.aggregate([
          ...
          { $unwind: "$chi_tiet" },
          {
            $group: {
              ...
              totalRevenue: {
                $sum: { $multiply: ["$chi_tiet.so_luong", "$chi_tiet.gia"] },
              },
            },
          },
          { $sort: { totalSold: -1 } },
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message: "Vui lòng cung cấp đủ thông tin khách hàng và sản phẩm.",
        });
      }
      if (!email) {
        return res
          .status(400)
          .json({ success: false, message: "Email khách hàng là bắt buộc." });
      }
      const customer = await NguoiDung.findOne({ email: email.toLowerCase() });
      if (!customer) {
        return res.status(404).json({
          success: false,
          message: `Không tìm thấy khách hàng với email: ${email}.`,
        });
      }
      ...
      for (const item of chi_tiet) {
        const product = await SanPham.findOne({
          "variants.sku": item.id_variant,
        });
        if (!product)
          return res.status(404).json({
            success: false,
            message: `Không tìm thấy sản phẩm với SKU: ${item.id_variant}`,
          });
        const variant = product.variants.find((v) => v.sku === item.id_variant);
        if (!variant)
          return res.status(404).json({
            success: false,
            message: `SKU ${item.id_variant} không tồn tại.`,
          });
        if (variant.so_luong < item.so_luong)
          return res.status(400).json({
            success: false,
            message: `Sản phẩm ${variant.sku} không đủ số lượng.`,
          });
...
router.put(
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      const { trang_thai_don_hang, trang_thai_thanh_toan, id_shipper } =
        req.body;
      ...
      if (trang_thai_don_hang) {
        const validOrderStatuses = DonHang.schema.path(
          "trang_thai_don_hang"
        ).enumValues;
        ...
        if (
          trang_thai_don_hang === "Hủy đơn hàng" &&
          orderToUpdate.trang_thai_don_hang !== "Hủy đơn hàng"
        ) {
          ...
          if (orderToUpdate.id_voucher) {
            await Voucher.findByIdAndUpdate(orderToUpdate.id_voucher, {
              is_active: true,
            });
          }
        }
      ...
      if (trang_thai_thanh_toan) {
        const validPaymentStatuses = DonHang.schema.path(
      ...
      if (id_shipper !== undefined) {
        if (id_shipper !== null && id_shipper !== "") {
          if (!mongoose.Types.ObjectId.isValid(id_shipper)) {
            return res
              .status(400)
              .json({ success: false, message: "ID shipper không hợp lệ." });
          }
          const shipperExists = await NguoiDung.findOne({
            _id: id_shipper,
            vai_tro: "shipper",
          });
          if (!shipperExists) {
            return res.status(400).json({
              success: false,
              message: "Shipper không tồn tại hoặc không có vai trò hợp lệ.",
            });
          }
        }
      ...
      if (
        ...
        const fullOrderDetails = await DonHang.findById(
          updatedOrder._id
        ).lean();
      ...
      const finalOrder = await DonHang.findById(updatedOrder._id)
        ...
        .populate("id_shipper", "ho_ten email")
        .lean();
